"""DETR transformer and position encoding utilities."""
...
class PositionEmbeddingSine(nn.Module):
    """Sine/cosine positional embeddings for 2D feature maps."""
    ...
    def forward(self, tensor: Tensor) -> Tensor:
        ...
        y_embed = torch.arange(h, device=tensor.device, dtype=torch.float32).view(1, h, 1)
        y_embed = y_embed.repeat(b, 1, w)
        x_embed = torch.arange(w, device=tensor.device, dtype=torch.float32).view(1, 1, w)
        x_embed = x_embed.repeat(b, h, 1)
        ...
        dim_t = torch.arange(
            self.num_pos_feats, dtype=torch.float32, device=tensor.device
        )
...
class Transformer(nn.Module):
    ...
    def forward(
        ...
        command_embedding: Tensor | None = None,
    ) -> Tensor:
        if len(src.shape) == 4:
            bs, _, _, _ = src.shape
